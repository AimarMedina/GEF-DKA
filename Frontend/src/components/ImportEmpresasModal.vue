...
async function descargarPlantilla() {
  try {
    loading.value = true
    const response = await api.get('/api/empresas/import/template', {
      responseType: 'blob'
    })
    ...
    link.href = url
    link.setAttribute('download', `plantilla_empresas_${new Date().toISOString().split('T')[0]}.csv`)
...
async function importarEmpresas() {
  ...
  try {
    ...
    const response = await api.post('/api/empresas/import', formData, {
      headers: {
    ...
    if (response.data.status === 'success') {
      ...
      const fileInput = document.getElementById('csvFileInputEmpresas')
      ...
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title">
            <i class="bi bi-building"></i> Importar Empresas
          </h5>
          <button
            type="button"
            class="btn-close"
        ...
        <div class="modal-body">
          ...
          <div v-if="importResult" class="mb-3">
            <div class="alert alert-info">
              ...
              <ul class="mb-0 mt-2">
                <li><strong>Total empresas procesadas:</strong> {{ importResult.created + importResult.failed }}</li>
                <li><strong class="text-success">✓ Empresas creadas:</strong> {{ importResult.created }}</li>
                <li v-if="importResult.failed > 0" class="text-danger">
                  <strong>✗ Empresas fallidas:</strong> {{ importResult.failed }}
                </li>
              </ul>
            </div>
          ...
          <div class="card">
            <div class="card-body">
              ...
              <p class="text-muted small">
                <strong>Campos requeridos:</strong> CIF, Nombre, Direccion, Email, N_Tel
              </p>

              <div class="mb-3">
                <input
                  id="csvFileInputEmpresas"
                  type="file"
                  class="form-control"
                  accept=".csv,.txt"
