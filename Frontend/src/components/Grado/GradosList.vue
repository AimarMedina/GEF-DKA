...
<template>
  <div class="col-12 col-lg-4 mt-3">
    ...
    <div v-if="!loading" class="list-group shadow-sm">
      <div
        class="list-group-item text-white bg-indigo cursor-pointer"
      ...
      >
        ...
        <div class="d-flex gap-2 w-100 mt-2">
          <button
            class="btn btn-sm btn-outline-indigo flex-fill"
    ...
    <nav v-if="totalPages > 1 && !loading" class="mt-3">
      <ul class="pagination pagination-sm justify-content-center">
        <li class="page-item" :class="{ disabled: currentPage === 1 }">
          <button class="page-link" @click.prevent="fetchGrados(currentPage - 1)">&laquo;</button>
        </li>
        <li class="page-item disabled">
          <span class="page-link">{{ currentPage }} / {{ totalPages }}</span>
        </li>
        <li class="page-item" :class="{ disabled: currentPage === totalPages }">
          <button class="page-link" @click.prevent="fetchGrados(currentPage + 1)">&raquo;</button>
        </li>
      </ul>
    </nav>

    <!-- Modales -->
    <CrearGradoModal :show="mostrarModal" @close="mostrarModal = false" @created="crearGrado" />

    <TransversalModal :show="mostrarTransversalModal" @close="mostrarTransversalModal = false" />
