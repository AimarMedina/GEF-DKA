...
async function descargarPlantilla() {
  try {
    loading.value = true
    const response = await api.get('/api/grados/import/template', {
      responseType: 'blob'
    })
    ...
    link.href = url
    link.setAttribute('download', `plantilla_cursos_${new Date().toISOString().split('T')[0]}.csv`)
...
async function importarGrados() {
  ...
  try {
    ...
    const response = await api.post('/api/grados/import', formData, {
      headers: {
    ...
    if (response.data.status === 'success') {
      ...
      const fileInput = document.getElementById('csvFileInputGrados')
      ...
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title">
            <i class="bi bi-mortarboard"></i> Importar Cursos
          </h5>
          <button
            type="button"
            class="btn-close"
        ...
        <div class="modal-body">
          ...
          <div v-if="importResult" class="mb-3">
            <div class="alert alert-info">
              ...
              <ul class="mb-0 mt-2">
                <li><strong>Total cursos procesados:</strong> {{ importResult.created + importResult.failed }}</li>
                <li><strong class="text-success">✓ Cursos creados:</strong> {{ importResult.created }}</li>
                <li v-if="importResult.failed > 0" class="text-danger">
                  <strong>✗ Cursos fallidos:</strong> {{ importResult.failed }}
                </li>
              </ul>
            </div>
          ...
          <div class="card">
            <div class="card-body">
              ...
              <p class="text-muted small">
                <strong>Campos requeridos:</strong> nombre, curso
              </p>

              <div class="mb-3">
                <input
                  id="csvFileInputGrados"
                  type="file"
                  class="form-control"
                  accept=".csv,.txt"
        ...
        <div class="modal-footer">
          ...
          <button
            ...
            @click="importarGrados"
            :disabled="!file || loading"
          >
            <span v-if="loading" class="spinner-border spinner-border-sm me-2"></span>
            {{ loading ? 'Importando...' : 'Importar Cursos' }}
          </button>
        </div>
      </div>
    </div>
